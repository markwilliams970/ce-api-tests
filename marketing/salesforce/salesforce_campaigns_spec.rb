...
describe "Marketing Hub Salesforce Campaign CRUD Tests" do
  ...
  it "should create a campaign" do
        ...
        source_json_file     = "campaignsfdcmar.json"
    ...
    it "should update a campaign" do
        ...
        source_json_file     = "patchcampaignsfdcmar.json"
        
        # Create a request object/hash that we can use to call cloud-elements server
        contact_request      = @sfdc_spec_helper.make_req(:patch, "marketing", "campaigns", "v2", source_json_file, $campaign_id)
    ...
    it "should find a campaign by ID" do
        ...
        source_json_file     = "findacampaignsfdc.json"
    
    	# Create a request object/hash that we can use to call cloud-elements server
        contact_request      = @sfdc_spec_helper.make_req(:get, "marketing", "campaigns", "v2", source_json_file, $campaign_id)
    ...
    it "should delete a campaign by ID" do
        ...
        # However, we're going to follow a functional naming scheme so that our
        # response output filename makes sense
        source_json_file     = "deleteacampaignsfdc.json"
        
        # Create a request object/hash that we can use to call cloud-elements server
        contact_request      = @sfdc_spec_helper.make_req(:delete, "marketing", "campaigns", "v2", source_json_file, $campaign_id)
