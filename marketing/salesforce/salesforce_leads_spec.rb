...
describe "Marketing Hub Salesforce Lead CRUD Tests" do
  ...
  it "should create a lead" do
        ...
        source_json_file     = "leadsfdcmar.json"
        
        # Create a request object/hash that we can use to call cloud-elements server
        contact_request      = @sfdc_spec_helper.make_req(:post, "marketing", "leads", "v2", source_json_file)
    ...
    it "should update a lead" do
        ...
        source_json_file     = "patchleadsfdcmar.json"
        
        # Create a request object/hash that we can use to call cloud-elements server
        contact_request      = @sfdc_spec_helper.make_req(:patch, "marketing", "leads", "v2", source_json_file, $lead_id)
    ...
    it "should find a lead by ID" do
        ...
        source_json_file     = "findaleadsfdc.json"
    
    	# Create a request object/hash that we can use to call cloud-elements server
        contact_request      = @sfdc_spec_helper.make_req(:get, "marketing", "leads", "v2", source_json_file, $lead_id)
    ...
    it "should delete a lead by ID" do
        ...
        # However, we're going to follow a functional naming scheme so that our
        # response output filename makes sense
        source_json_file     = "deletealeadsfdc.json"
        
        # Create a request object/hash that we can use to call cloud-elements server
        contact_request      = @sfdc_spec_helper.make_req(:delete, "marketing", "leads", "v2", source_json_file, $lead_id)
